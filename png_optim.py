...
def pngquant(path: Path) -> Path:
    ...
    cmd = f"pngquant -f -o {quote(str(outfile))} --speed 1 --quality 65-80 --strip --skip-if-larger {quote(str(path))}"
    os.system(cmd)
    return outfile
...
def handle_png_files(p_queue: Queue, use_pngquant: bool):
    """pngquant + optipng"""
    while p_queue.empty() is False:
        ...
        if use_pngquant is True:
            # zopflipng
            optimized = zopflipng(original_png_file)
            if optimized.exists():
...
if __name__ == "__main__":
    ...
    parser.add_argument("-q", "--quant", dest="quant", action="store_true", help="Use pngquant (png8b)")
    ...
    # Optimize
    t = common.parallel(handle_png_files, (queue, args.quant, ))
